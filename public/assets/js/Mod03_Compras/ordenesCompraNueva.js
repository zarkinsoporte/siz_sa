...
//Tablas
var COL_PARTIDA = 0;
var COL_BTN_ELIMINAR_COMPRA = 1;
var COL_PARTIDA_CERRADA = 2;
...
var COL_IVA = 13;
var COL_MONTO_IVA = 14;
var COL_TOTAL = 15;
var COL_FECHA_ENTREGA_COMPRA = 16;
var COL_CANT_PENDIENTE = 17;
var COL_ID_IVA = 20;
var COL_ID_PARTIDA = 21;
var COL_ESTATUS_PARTIDA = 22;
...
function InicializaTablas(){
    ...
    TBL_ART_MISC = $('#tblArticulosMiscelaneosNueva').DataTable({
        ...
        "columnDefs": [
            ...
            {
                "targets": [COL_MONTO_IVA], //monto IVA
                "orderable" : false
            },
            {
                "targets": [COL_TOTAL], //total
                "orderable" : false
            },
            {
                "targets": [COL_FECHA_ENTREGA_COMPRA],
...
function cargaArticulo(articulo, pos){
    ...
    if (count > 0){
        ...
        $('#ordenCompra-subtotal').text(
            Intl.NumberFormat("es-MX", {
                minimumFractionDigits: TOTAL_DECIMALES
            }).format(TSubtotal));
        $('#ordenCompra-descuento').text(Intl.NumberFormat("es-MX", {
            minimumFractionDigits: TOTAL_DECIMALES
        }).format(Tdescuento));
        $('#ordenCompra-iva').text(Intl.NumberFormat("es-MX", {
            minimumFractionDigits: TOTAL_DECIMALES
        }).format(Tiva));
        $('#ordenCompra-total').text(SIMBOLO_MONEDA + ' ' + Intl.NumberFormat("es-MX", {
            minimumFractionDigits: TOTAL_DECIMALES
        }).format(Ttotal));
...
function CargaComponentesOC(resumen) {
    ...
    if (resumen.OC_ESTATUS == 'CERRADA'){
        $('#input_tc').attr("disabled", "disabled");
        $('#guardar').attr("disabled", "disabled");
        $('#guardar').attr("id", "guardar2");
        $('#cboMoneda').attr("disabled", "disabled");
        $('#input-comentarios').attr("disabled", "disabled");
        $('#input-fecha-entrega').attr("disabled", "disabled");
    }else{
        $('#input_tc').removeAttr("disabled");
        $('#guardar2').attr("id", "guardar");
        $('#guardar2').removeAttr("disabled");
        $('#cboMoneda').removeAttr("disabled");
        $('#input-fecha-entrega').removeAttr("disabled");
        $('#input-comentarios').removeAttr("disabled");
    }
...
function agregaArtExis2(datos, pos) {
...
function set_columns_index(BanderaOC) {
    if (BanderaOC == 0) {
        ...
        COL_DESCUENTO = 8;
        COL_MONTO_DESCUENTO = 9;
        COL_IVA = 10;

        COL_MONTO_IVA = 11;
        COL_TOTAL = 12;
        COL_FECHA_ENTREGA_COMPRA = 13;
        COL_CANT_PENDIENTE = 14;
        COL_ID_IVA = 15;
